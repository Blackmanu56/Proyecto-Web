  ...
  try {
    ...
    const [ventas, compras, movimientos, historiales, cajasAbiertas] = await Promise.all([
      ...
      prisma.historialEstado.findMany({
        ...
        select: { id: true, fecha: true, usuarioId: true, productoId: true, estadoAnterior: true, estadoNuevo: true },
      }),
      prisma.caja.groupBy({
        by: ["usuarioId"],
    ...
    for (const h of historiales) {
      ...
      items.push({
        ...
        descripcion: `Producto #${h.productoId}: ${h.estadoAnterior} → ${h.estadoNuevo}`,
      });
    }
